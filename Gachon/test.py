def find_mapping(encoded_strings, target_string):
    ...
    encoded_words_lists = [list(''.join(line.split())) for line in encoded_strings]
    target_words = list(''.join(target_string.split()))

    for encoded_words in encoded_strings:
        ...
        if len(encoded_words) == len(target_words):
            ...
            if all(is_mapping_consistent(''.join(line.split()), mapping) for line in encoded_strings):
                return mapping
...
def decode_strings(encoded_strings, mapping):
    return [''.join(mapping.get(ew, ' ') for ew in line) for line in encoded_strings]

# 사용자로부터 인코딩된 문자열을 입력받습니다.
encoded_strings = []
print("Enter encoded strings, end with a blank line:")
while True:
    line = input()
    if line:
